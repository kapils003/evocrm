    ...
    <?php if (have_posts()): while (have_posts()) : the_post(); 
        ...
        <div class="inner-page row wp_page<?php echo (isset($sidebar) && !empty($sidebar) ? " is_sidebar" : " no_sidebar"); ?>">
        	...
        	<div id="post-<?php the_ID(); ?>" <?php echo post_class($content_class . " page-content post-entry"); ?>>
                ...
                <?php 
                wp_reset_postdata();
                echo "<div class='wp-clock-date'>";
                echo do_shortcode('[coolclock radius="100" showdigital=digital12 /]');
                echo date('l jS \of F Y');
                echo "</div>";
                echo "<div class='wp-evo-logo'>";
                echo "<img src='http://www.evocrm.co.uk/wp-content/uploads/2016/11/Website-Logo-2.png' alt='evo-logo' class='evo-logo'>";
                echo "</div>";
                ...
                echo "Total Customers ".$count_customers->publish."";
                echo "</div>";
                echo "<div class='wp-total-sales wp-home-total'>";
                $query = new WP_Query( array( 'post_type' => 'listings', 'meta_key' => 'car_sold', 'meta_value' => "1" ) );
                echo "Total Sales ".$query->found_posts."";
                echo "</div>";
                echo "<div class='wp-total-vehicles wp-home-total'>";
                echo "Total Vehicles in stock ".$count_vehicles->publish."";
                echo "</div>";
                echo "<div class='wp-home-sticky-notes'>";
                echo "Sticky Notes";
                $user_id = get_current_user_id();
                $stickyNotes = get_user_meta($user_id, 'sticky-notes', 'true');
                $sticky = '<form  id="stick-notes-form" method="post">';
				$sticky.= '<textarea name="stickynotes" placeholder="Write your Sticky Notes here" cols="20" rows="5">' .$stickyNotes. '</textarea>';
